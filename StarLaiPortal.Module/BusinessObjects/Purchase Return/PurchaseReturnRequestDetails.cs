...
namespace StarLaiPortal.Module.BusinessObjects.Purchase_Return
{
    ...
    [RuleCriteria("PRRStockNoQOH", DefaultContexts.Save, "IsValid = 0", "Bin not enough stock.")]
    [RuleCriteria("PRRStockNoPrice", DefaultContexts.Save, "IsValid1 = 0", "Price not allow 0.")]

    [XafDisplayName("Purchase Return Request Details")]
    public class PurchaseReturnRequestDetails : XPObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        // Use CodeRush to create XPO classes and properties with a few keystrokes.
        // https://docs.devexpress.com/CodeRushForRoslyn/118557
        public PurchaseReturnRequestDetails(Session session)
            : base(session)
        {
        }
        ...
        public bool IsValid
        {
            get
            {
                if (this.BaseDoc == null || !IsNew)
                {
                    ...
                    if (binstock != null)
                    {
                        if (binstock.InStock < this.RtnQuantity)
                        {
                            return true;
                        }
                    }
